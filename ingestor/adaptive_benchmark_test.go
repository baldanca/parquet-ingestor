...
func newAdaptiveBenchmarkIngestor(b *testing.B, pollers, used, cap int) (*Ingestor[int], *adaptiveSource) {
	...
	ing, err := NewIngestor[int](cfg, src, tTransformer{}, enc, sk, func(ctx context.Context, bt batcher.Batch[int]) (string, error) { return "k", nil })
	if err != nil {
		b.Fatal(err)
	}
...
func BenchmarkIngestorAdaptiveScaleWorkersUpDown(b *testing.B) {
	...
	ing, err := NewIngestor[int](cfg, src, tTransformer{}, &tEncoder{ct: "application/octet-stream", ext: ".bin"}, &tSink{}, func(ctx context.Context, bt batcher.Batch[int]) (string, error) { return "k", nil })
	if err != nil {
		b.Fatal(err)
	}
	ing.SetMetricsRegistry(&observability.Registry{})
