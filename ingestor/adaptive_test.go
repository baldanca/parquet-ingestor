...
func TestIngestor_SetFlushWorkers_ResizesPool(t *testing.T) {
	...
	ing, err := NewIngestor[int](cfg, src, tTransformer{}, enc, sk, func(ctx context.Context, b batcher.Batch[int]) (string, error) {
		return "k", nil
	})
	if err != nil {
		t.Fatal(err)
...
func TestIngestor_AdaptiveDecision_ScalesUpWorkersAndPollers(t *testing.T) {
	...
	cfg.FlushInterval = time.Second
	cfg.MaxEstimatedInputBytes = 1024
	ing, err := NewIngestor[int](cfg, src, tTransformer{}, enc, sk, func(ctx context.Context, b batcher.Batch[int]) (string, error) { return "k", nil })
	if err != nil {
		t.Fatal(err)
	}
